...
## CommonJS, UMD 모듈 지원(지원 예정)

ExecuteJS는 CommonJS, UMD 형식의 패키지를 자동으로 ES 모듈로 변환합니다.
...
3. **CommonJS 변환**: CommonJS 형식의 패키지는 자동으로 ES 모듈로 변환됩니다.
4. **모듈 로딩**: Deno Core를 통해 모듈을 로드하고 실행합니다.
...
현재는 **ESM 형식의 패키지**만 지원합니다. CommonJS, UMD 전용 패키지는 제한적으로 지원될 수 있습니다.
